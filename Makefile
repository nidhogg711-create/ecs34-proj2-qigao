...
# Define the flags
DEFINES         =
INCLUDE         += -I $(INC_DIR) -I $(shell brew --prefix)/include
CFLAGS          +=
CPPFLAGS        += -std=c++20
LDFLAGS         =
...
TEST_LDFLAGS    = $(LDFLAGS) -L $(shell brew --prefix)/lib -lgtest -lgtest_main -lpthread
...
gencoverage:
	...
	lcov --remove $(TESTCOVER_DIR)/coverage.info '/usr/*' '*/testsrc/*' '$(shell brew --prefix)/*' --output-file $(TESTCOVER_DIR)/coverage.info --ignore-errors inconsistent,unsupported,format
	genhtml $(TESTCOVER_DIR)/coverage.info --output-directory $(TESTCOVER_DIR) --ignore-errors inconsistent,corrupt,unsupported,category
